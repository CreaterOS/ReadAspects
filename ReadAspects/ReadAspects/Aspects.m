 ...
 * 切面运行锁
 * 参数: block回调
 * 参考链接: https://www.jianshu.com/p/a6ed560a267e
 *
     //  OSSpinLock自旋锁的初始化
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    NSLog(@"klass name:%@, class name:%@",NSStringFromClass(klass),NSStringFromClass(self.class));
    //获得目标方法
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    ...
    NSLog(@"statedClass:%@, class name:%@",NSStringFromClass(statedClass),className);
...
static AspectsContainer *aspect_getContainerForObject(NSObject *self, SEL selector) {
    NSCParameterAssert(self);
    SEL aliasSelector = aspect_aliasForSelector(selector);
    NSLog(@"aliasSelector:%@",NSStringFromSelector(aliasSelector));
    
    AspectsContainer *aspectContainer = objc_getAssociatedObject(self, aliasSelector);
    NSLog(@"aspectContainer:%@",aspectContainer);
    if (!aspectContainer) {
        aspectContainer = [AspectsContainer new];
        //aspectSelector --> aspectContainer
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    //获得selector名称
    //selector名称不能是retain,release,autorelease,forwardInvocation:
...
- (void)addAspect:(AspectIdentifier *)aspect withOptions:(AspectOptions)options {
    ...
    NSLog(@"self.beforeAspects:%@, self.insteadAspects:%@, self.afterAspects:%@",self.beforeAspects,self.insteadAspects,self.afterAspects);
}
